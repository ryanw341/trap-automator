...
class TrapAutomator {
  ...
  openEditTriggerDialog() {
    ...
    const trigMap = {};
    // Build trigger mapping. Combine builtin and custom triggers. For categories
    // that belong to the grimdark family (imperial, eldar, ork, etc.), map
    // all triggers into the primary key "grimdark" so that there is only one
    // category shown in the UI. This also prevents duplicate category entries.
    if (this.definitions.triggers) {
      for (const cat of Object.keys(this.definitions.triggers)) {
        ...
        const mapKey = primary === 'grimdark' ? 'grimdark' : cat;
        if (!trigMap[mapKey]) trigMap[mapKey] = [];
        trigMap[mapKey] = trigMap[mapKey].concat(this.definitions.triggers[cat]);
      }
    }
    try {
      const custom = game.settings.get('trap-automator', 'customDefs') || {};
      if (custom.triggers) {
        for (const cat of Object.keys(custom.triggers)) {
          ...
          const mapKey = primary === 'grimdark' ? 'grimdark' : cat;
          if (!trigMap[mapKey]) trigMap[mapKey] = [];
          trigMap[mapKey] = trigMap[mapKey].concat(custom.triggers[cat]);
        }
      }
    } catch (err) {}
    // Deduplicate triggers for each category
    for (const k of Object.keys(trigMap)) {
      const seen = new Set();
      trigMap[k] = trigMap[k].filter(t => {
